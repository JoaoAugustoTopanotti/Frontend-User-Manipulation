...
function ListUsers({ users, onOpenModal, onDeleteModal, setOrderByField, setOrderByDirection }: { users: IUser[], onOpenModal: (user: IUser, editMode: boolean) => void; onDeleteModal: (userId: string | undefined) => void; setOrderByField: (field:string) => void; setOrderByDirection: React.Dispatch<React.SetStateAction<'asc' | 'desc'>>;}){
  return (
    <>
              ...
              <tr>
                <th className="px-4 py-3 w-2/11">
                  <div className="flex items-center gap-1">
                    Nome
                    <button className="p-1 justify-center items-center cursor-pointer" onClick={() => {setOrderByField("name"), setOrderByDirection((prev: 'asc' | 'desc') => (prev === 'desc' ? 'asc' : 'desc'))}}>
                      <FaCaretDown />
                    </button>
                  </div>
                </th>
                <th className="px-4 py-3 w-3/11">
                  <div className="flex items-center gap-1">
                    Email
                    <button className="p-1 cursor-pointer" onClick={() => {setOrderByField("email"), setOrderByDirection((prev: 'asc' | 'desc') => (prev === 'desc' ? 'asc' : 'desc'))}}>
                      <FaCaretDown />
                    </button>
                  </div>
                </th>
                <th className="px-4 py-3 w-2/11">
                  <div className="flex items-center gap-1">
                    Data de Nascimento
                    <button className="p-1 cursor-pointer" onClick={() => {setOrderByDirection((prev: 'asc' | 'desc') => (prev === 'desc' ? 'asc' : 'desc'))}}>
                      <FaCaretDown />
                    </button>
                  </div>
                </th>
                ...
                <th className="px-4 py-3 w-2/11">
                  <div className="flex items-center gap-1">
                    CPF
                    <button className="p-1 cursor-pointer" onClick={() => {setOrderByField("nationalId"), setOrderByDirection((prev: 'asc' | 'desc') => (prev === 'desc' ? 'asc' : 'desc'))}}>
                      <FaCaretDown />
                    </button>
                  </div>
                </th>
        ...
        <div className="w-5/6 flex items-center justify-between font-medium text-sm  gap-4">
          <div className="flex flex-col items-start pb-4 mx-auto">
            <input
              ...
              className="w-2xl border border-gray-300 focus:outline-none hover:border-blue-500 rounded-2xl px-4 py-1"
            />
          </div>
          <UserModal
            user={selectedUser}
